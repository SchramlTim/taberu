...
export const FormProvider: FC = ({ children }) => {
    const [fieldErrors, setFieldErrors] = useState<FormFieldError[]>(
        contextDefaultValues.state.fieldErrors
    )
    const [globalErrors, setGlobalErrors] = useState<Error[]>(
        contextDefaultValues.state.globalErrors
    )

    const addFieldError = (identifier: string, error: Error) => {
        const fieldErrorExist = !!fieldErrors.filter(
    ...
    const addGlobalError = (error: Error) => {
        const globalErrorExist = !!globalErrors.filter(
    ...
    const removeFieldError = (identifier: string) => {
        const errorList = fieldErrors.filter(
            (entry) => entry.identifier !== identifier
        )
    ...
    return (
        <FormContext.Provider
            value={{
                state: { fieldErrors, globalErrors },
                addFieldError,
                addGlobalError,
                removeFieldError,
            }}
