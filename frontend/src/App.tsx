...
function App() {
    ...
    Notification.requestPermission(function (status) {
        if (status == 'granted') {
            get(
                process.env.REACT_APP_API_ENDPOINT + '/v1/notification/token'
            ).then((response) => {
                createNotificationSubscription(response.data.token).then(
            ...
            <UserProvider>
                <DefaultLayout>
                    <Switch>
                        ...
                        <PrivateRoute
                            path="/bowls/create"
                            component={BowlCreate}
                            exact
                        />
                        <PrivateRoute
                            path="/bowls/:id"
                            component={BowlDetails}
                            exact
                        />
                    </Switch>
                </DefaultLayout>
