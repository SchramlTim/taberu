...
function OrderDetail(props: { order: OrderProps }) {
    ...
    const togglePaymentStatus = () => {
        if (user?.id === bowl?.creatorId) {
            const toggleStatus =
                order.paymentStatus === 'PAID' ? 'NOT_PAID' : 'PAID'
            patch(
    ...
    useEffect(() => {
        get(
            process.env.REACT_APP_API_ENDPOINT + '/v1/users/' + order.userId
        ).then((response) => {
                ...
                {orderItems &&
                    orderItems.map((item, index) => (
                        <li key={index} className={'flex flex-col'}>
                            <div
                                className={
                                    'grid grid-cols-[10%,20%,60%] w-full '
                                }
                            >
