...
import OrderableItemList, {
    LoadingOrderableItemList,
} from './OrderableItemList/OrderableItemList'
...
function BowlDetails() {
    ...
    useEffect(() => {
        get(process.env.REACT_APP_API_ENDPOINT + '/v1/bowls/' + id).then(
            (response) => {
                setBowls(response.data)
                get(response.data.orders).then((response) => {
            ...
            <BasketProvider bowl={bowl}>
                ...
                >
                    ...
                    >
                        ...
                        <div
                            className={
                                'flex justify-between gap-5 mt-10 md:m-0 md:flex-col md:w-1/3'
                            }
                        >
                    ...
                    <div
                        className={
                            'flex flex-col justify-between gap-3 w-11/12 lg:w-3/4 mt-4'
                        }
                    >
...
function LoadingTitle() {
    return (
        <span className={'text-2xl w-full flex animate-pulse'}>
            <span className={'bg-gray-300 w-3/4 h-10 rounded-lg'}></span>
        </span>
    )
}
...
function LoadingDescription() {
    return (
        <span className={'text-2xl w-full flex animate-pulse'}>
            <span className={'bg-gray-300 w-full h-16 rounded-lg'}></span>
        </span>
    )
}
