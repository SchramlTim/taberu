...
function MenuCreate() {
    ...
    const createMenu = async () => {
        const response = await post(
            process.env.REACT_APP_API_ENDPOINT + '/v1/menus',
            {
                name: menuName,
        ...
        return (
            <>
                ...
                >
                    <div
                        className={
                            'flex flex-col w-full max-w-[80%] bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4'
                        }
                    >
                    ...
                    <div
                        className={
                            'flex flex-col gap-1 w-full max-w-[80%] mt-3'
                        }
                    >
                        {menuItems.map((item) => {
                            return (
                                <div
                                    key={item.id}
                                    className={
                                        'flex gap-2 flex-wrap justify-between p-2 rounded bg-gray-200 w-full'
                                    }
                                >
                                    <span className={'w-1/3 break-all'}>
                                        {item.name}
                                    </span>
                                    <span
                                        className={'w-1/4 break-all text-right'}
                                    >
                                        {item.description}
                                    </span>
                                    <span
                                        className={
                                            'w-1/4 break-all text-gray-600 text-center'
                                        }
                                    >
                                        {item.price}
                                    </span>
