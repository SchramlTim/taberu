            ...
            <div className={'w-full max-w-xs'}>
                <FormProvider>
                    <Form name={'login'} method={'POST'} action={process.env.REACT_APP_API_ENDPOINT + "/v1/users/register"} afterSubmit={() => setAfterRegister(true)} className={'bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4'}>
                        <Input identifier={'username'} title={'Name*'} autocomplete={'email'} placeholder={''} validation={(input) => {
                            validateNotEmpty(input)
                            validateEmail(input)
                        }}/>
                        <Input identifier={'firstName'} title={'First Name*'} autocomplete={'given-name'} placeholder={''} validation={(input) => {
                            validateNotEmpty(input)
                        }}/>
                        <Input identifier={'lastName'} title={'Last Name*'} autocomplete={'family-name'} placeholder={''} validation={(input) => {
                            validateNotEmpty(input)
                        }}/>
                        <Input identifier={'password'} title={'Password*'} autocomplete={'new-password'} placeholder={'*****************'} type={'password'} validation={(input) => {
                            validateNotEmpty(input)
