...
export const Bowls = () => {
    ...
    return (
        <>
                ...
                <div className={'lg:w-3/4 w-11/12'}>
                    {!bowls ? (
                        Array(3)
                            .fill(null)
                            .map((element, index) => (
                                <LoadingBowlListItem key={index} />
                            ))
                    ) : bowls.length ? (
                        bowls.map((bowl: BowlProps, index) => (
            ...
            <Link
                to="/bowls/create"
                className="bg-button-primary fixed bottom-10 right-10 text-black text-center py-2 px-4 rounded h-14 w-14 inline-flex items-center"
            >
                <svg
